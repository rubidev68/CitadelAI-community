...
import { LLMProvider, LLMConfig, getLLMConfig, PROVIDER_CONFIGS, CustomProviderConfig, LLMResponse } from './types';
...
export class LLMService {
  ...
  async generateResponse(
    ...
    for (let i = 0; i < fallbackProviders.length; i++) {
      ...
      try {
        ...
        // Log usage
        if (response.usage) {
          await this.logUsage(chatbotId, service.modelName, provider, 'CHAT', response.usage, userId);
        }
  ...
  async generateStreamingResponse(
    ...
    for (let i = 0; i < fallbackProviders.length; i++) {
      ...
      try {
        ...
        // Call provider directly without fallback recursion
        const responseObj = await service._generateStreamingResponseDirect(
          systemPrompt,
          history,
          augmentedUserMessage,
          res
        ...
        logger.debug('Provider streaming completed', {
          responseLength: responseObj.content.length,
          service: 'llmService',
        });

        // Log usage
        if (responseObj.usage) {
          await this.logUsage(chatbotId, service.modelName, provider, 'CHAT', responseObj.usage, userId);
        }
  ...
  private async logUsage(
    ...
    try {
        // Fetch chatbot to get adminUserId
        ...
        // Find GlobalAIModel
        const globalModel = null; // Community Edition: No global models
        ...
        // Community Edition: No usage logging
    } catch (err) {
        // Silently fail logging to not disrupt chat flow, but log error
        logger.error('Failed to log AI usage', err instanceof Error ? err : new Error(String(err)));
    }
  }
