...
const client: WeaviateClient | null = (() => {
  ...
  const apiKey = config.WEAVIATE_API_KEY;
...
async function updateWebsiteContentSchema() {
  ...
  try {
    ...
    if (websiteContentExists) {
      ...
      if (currentSchema.vectorizer !== 'text2vec-transformers') {
        logger.warn('WebsiteContent schema exists but vectorizer is not text2vec-transformers', {
        ...
        logger.info('WebsiteContent schema already has text2vec-transformers vectorizer configured', {
          service: 'updateWeaviateSchemas',
        });
        return;
      }
    ...
    logger.info('WebsiteContent schema created/updated with text2vec-transformers vectorizer', {
      service: 'updateWeaviateSchemas',
    });
  } catch (error: unknown) {
...
async function updateDocumentContentSchema() {
  ...
  try {
    ...
    if (documentContentExists) {
      ...
      if (currentSchema.vectorizer !== 'text2vec-transformers') {
        logger.warn('DocumentContent schema exists but vectorizer is not text2vec-transformers', {
          currentVectorizer: currentSchema.vectorizer || 'none',
          service: 'updateWeaviateSchemas',
        });
        ...
        logger.info('DocumentContent schema already has text2vec-transformers vectorizer configured', {
          service: 'updateWeaviateSchemas',
        });
        return;
      }
    ...
    await client.schema.classCreator().withClass(schemaConfig).do();
    logger.info('DocumentContent schema created/updated with text2vec-transformers vectorizer', {
      service: 'updateWeaviateSchemas',
    });
  } catch (error: unknown) {
