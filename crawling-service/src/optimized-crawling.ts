...
const RESOURCE_EXTENSIONS = [
  ...
  // Documents (supported types removed from exclusion list)
  // '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx', '.odt', '.ods', '.odp', - Now supported
  // Archives
  '.zip', '.rar', '.tar', '.gz', '.7z',
  // Media
...
function isResourceFile(url: string): boolean {
  try {
    ...
    // Special handling for OpenDocument formats
    if (pathname.endsWith('.odt') || pathname.endsWith('.ods') || pathname.endsWith('.odp')) {
...
// Initialize semantic chunking service - REMOVED (using shared service)
...
class OptimizedCrawlingService {
  ...
  private async processBatch() {
    ...
    try {
      // Process items in the batch sequentially to avoid overwhelming the vectorizer
  ...
  private async processContentItem(item: ContentBatch) {
    try {
      ...
      // Fetch chatbot owner for usage logging
      const chatbot = await prisma.chatbot.findUnique({
        where: { id: websiteContext.chatbotId },
        select: { ownerId: true }
      });

      if (!chatbot) {
        console.log(`[CRAWL SKIP] - Chatbot not found for id: ${websiteContext.chatbotId}`);
        return;
      }
      ...
      // Use shared document processor
      // Disable summary generation for HTML/web content as requested to save costs
      ...
      try {
        processedDoc = await docProcessor.processDocument(
          ...
          { 
            ...
            chunkingOptions: { contentType: 'web' }
          }
        );
        ...
        // Usage logging disabled in Community Edition
         ...
         console.error(`[CRAWL ERROR] - Document processing failed for ${item.url}`, err);
         throw err;
      }

      const semanticChunks = processedDoc.chunks;
        ...
        if (summary) {
          try {
             await client.data
              ...
              .do();
              console.log(`[CRAWL WEAVIATE] - Stored summary for: ${item.url}`);
          } catch (e) {
             console.error(`[CRAWL WEAVIATE ERROR] - Failed to store summary for ${item.url}`, e);
          }
        }

        // Process in batches to avoid overwhelming the vectorizer
        const BATCH_SIZE = 2;
        for (let i = 0; i < semanticChunks.length; i += BATCH_SIZE) {
          const batch = semanticChunks.slice(i, i + BATCH_SIZE);
          const chunkPromises = batch.map((chunk: any) => 
            client.data
              .creator()
              .withClassName('WebsiteContent')
              .withProperties({
