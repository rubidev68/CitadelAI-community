...
import { DocumentProcessorService } from '@shared/services';
...
export async function handleProcessDocument(req: AdminAuthRequest & { file?: Express.Multer.File }, res: Response): Promise<void> {
  try {
    ...
    if (!file) {
      res.status(400).json({ error: 'No file provided or invalid file type. Allowed types: PDF, Word, Excel, PowerPoint, OpenDocument, CSV, Text.' });
      return;
    }

    // Additional file type validation
    // (multer filter already handled this, but good to be explicit if needed, 
    // or we can remove strict check if we trust multer or want to support more)
    
    // Validate file magic bytes (PDF signature) - ONLY for PDF
    if (file.mimetype === 'application/pdf' && file.buffer.toString('ascii', 0, 4) !== '%PDF') {
    ...
    // Process document using Shared Service
    const docProcessor = new DocumentProcessorService(process.env.GEMINI_API_KEY);
    let processedDoc;
    ...
    try {
      ...
      documentsLogger.error('Document processing error', { error: processError instanceof Error ? processError : new Error(String(processError)) });
      if (!res.headersSent) {
        res.status(400).json({ error: 'Failed to process document. ' + (processError instanceof Error ? processError.message : 'Unknown error') });
      }
      return;
    }
    ...
    if (!processedDoc.text.trim()) {
      res.status(400).json({ error: 'File appears to be empty or contains no readable text.' });
      return;
    }

    // Vectorize the content (store chunks in Weaviate)
    let vectors: Array<{ content: string; metadata: Record<string, unknown> }> = [];
    try {
      vectors = await vectorizeContent(
        processedDoc.chunks, 
        chatbotId, 
        blockId, 
        file.originalname, 
    ...
    res.json({
      markdown: processedDoc.text, // Frontend might expect 'markdown' but it's really just the extracted text
      vectors,
