...
export const getCloudIndexedFiles = async (
  chatbotId: string,
  blockId: string,
  token: string
): Promise<Array<{ fileName: string; filePath: string; fileType: string; mimeType?: string; fileSize?: number; modifiedAt?: string; url?: string }>> => {
  const response = await adminApiClient.get(`/cloud/integration/${blockId}/indexed-files?chatbotId=${chatbotId}`, token);
  return handleApiResponse(response) as Promise<Array<{ fileName: string; filePath: string; fileType: string; mimeType?: string; fileSize?: number; modifiedAt?: string; url?: string }>>;
};
...
export const listGlobalAIModels = async (token: string): Promise<GlobalAIModel[]> => {
  // Community Edition: Hardcoded models
  return Promise.resolve([
    { id: 'gpt-4o', provider: 'openai', modelId: 'gpt-4o', name: 'GPT-4o', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: true, isDeprecated: false, createdAt: new Date().toISOString() },
    { id: 'gpt-4-turbo', provider: 'openai', modelId: 'gpt-4-turbo', name: 'GPT-4 Turbo', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: false, isDeprecated: false, createdAt: new Date().toISOString() },
    { id: 'gpt-3.5-turbo', provider: 'openai', modelId: 'gpt-3.5-turbo', name: 'GPT-3.5 Turbo', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: false, isDeprecated: false, createdAt: new Date().toISOString() },
    { id: 'gemini-1.5-pro', provider: 'gemini', modelId: 'gemini-1.5-pro', name: 'Gemini 1.5 Pro', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: false, isDeprecated: false, createdAt: new Date().toISOString() },
    { id: 'gemini-1.5-flash', provider: 'gemini', modelId: 'gemini-1.5-flash', name: 'Gemini 1.5 Flash', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: false, isDeprecated: false, createdAt: new Date().toISOString() },
    { id: 'claude-3-opus', provider: 'anthropic', modelId: 'claude-3-opus-20240229', name: 'Claude 3 Opus', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: false, isDeprecated: false, createdAt: new Date().toISOString() },
    { id: 'claude-3-sonnet', provider: 'anthropic', modelId: 'claude-3-sonnet-20240229', name: 'Claude 3 Sonnet', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: false, isDeprecated: false, createdAt: new Date().toISOString() },
    { id: 'claude-3-haiku', provider: 'anthropic', modelId: 'claude-3-haiku-20240307', name: 'Claude 3 Haiku', inputCostPer1M: 0, outputCostPer1M: 0, isVisible: true, isDefault: false, isDeprecated: false, createdAt: new Date().toISOString() },
  ] as any[]); // Use as any to bypass strict type check during patch if needed
};
...
export const listCredentials = async (token: string, type?: string): Promise<IntegrationCredential[]> => {
  const query = type ? `?type=${encodeURIComponent(type)}` : '';
  const response = await adminApiClient.get(`/credentials${query}`, token);
  return handleApiResponse(response) as Promise<IntegrationCredential[]>;
};

export const createCredential = async (
  token: string,
  payload: { name: string; type: string; data: Record<string, unknown> }
): Promise<IntegrationCredential> => {
  const response = await adminApiClient.post('/credentials', payload, token);
  return handleApiResponse(response) as Promise<IntegrationCredential>;
};
...
export const updateCredential = async (
  ...
  const response = await adminApiClient.put(`/credentials/${id}`, payload, token);
  return handleApiResponse(response) as Promise<IntegrationCredential>;
};

export const deleteCredential = async (token: string, id: string): Promise<void> => {
  const response = await adminApiClient.delete(`/credentials/${id}`, token);
  return handleApiResponse(response) as Promise<void>;
};
