...
import { Cloud, CheckCircle2, XCircle, AlertCircle, ExternalLink, RefreshCw, Loader2, Folder, X, File, Save, Trash2 } from 'lucide-react';
import {
  startCloudOAuth,
  getCloudIntegration,
...
import CloudIndexedFilesModal from './CloudIndexedFilesModal';
...
const CloudBlockProperties: React.FC<CloudBlockPropertiesProps> = ({ block }) => {
  ...
  const properties = useMemo(() => (block.properties || {}) as CloudBlockProperties, [block.properties]);
  ...
  // Track loaded block ID to prevent re-fetching loop
  const loadedBlockIdRef = useRef<string | null>(null);

  // Update state when properties change (e.g., after loading integration)
  // Only sync status fields, NOT input fields, to avoid overwriting user input during typing/saving
  useEffect(() => {
    if (properties.isConnected !== undefined) {
      setIsConnected(properties.isConnected);
    }
    // We intentionally DO NOT sync autoRefresh here because it causes a feedback loop
    // when updateProperties is called. We trust the local state and only sync on initial load.
  ...
  // cronEnabled is synced with autoRefresh
  // const [cronEnabled, setCronEnabled] = useState<boolean>(properties.cronEnabled || false);
  ...
  const [isIndexedFilesModalOpen, setIsIndexedFilesModalOpen] = useState(false);
  ...
  const loadCredentials = React.useCallback(async () => {
    ...
    try {
      const data = await listCredentials(token, 'CLOUD');
      setCredentials(data);
    } catch (error) {
  ...
  // Try to match current configuration with an existing credential on load
  useEffect(() => {
    if (credentials.length > 0 && !selectedCredentialId && provider && !isConnected) {
      // Find a credential that matches current config
      const match = credentials.find(c => {
        // Basic match: provider
        if (c.data.provider !== provider) return false;
        
        // Detailed match depending on provider
        if (provider === 'nextcloud') {
          return c.data.baseUrl === baseUrl && c.data.username === username;
        } else if (provider === 'ssh') {
          return c.data.host === sshHost && c.data.username === sshUsername;
        }
        return false;
      });
      ...
      if (match) {
        ...
        setCredentialName(match.name);
      }
    }
  }, [credentials, selectedCredentialId, provider, baseUrl, username, sshHost, sshUsername, isConnected]);
  ...
  }, [credentialName, properties, provider, appPassword, sshPassword, privateKey, sshPassphrase, autoRefresh, cronSchedule]);
  ...
  const saveCredential = useCallback(async (overrides?: any) => {
    ...
    const connectionData = {
      ...
      // Secrets
      accessToken: overrides?.accessToken ?? appPassword, // For Nextcloud App Password
      privateKey: overrides?.privateKey ?? privateKey,
      passphrase: overrides?.passphrase ?? sshPassphrase,
      password: overrides?.password ?? sshPassword,
      // Cron settings
      autoRefresh: overrides?.autoRefresh ?? autoRefresh,
      cronSchedule: overrides?.cronSchedule ?? cronSchedule,
    };
  ...
  const handleDeleteCredential = async () => {
    ...
    try {
      ...
      // Reset properties to avoid confusion? 
      // Or keep them as they are but they are no longer linked to a credential
  ...
  */
  ...
  const handleCredentialSelect = (value: string) => {
    ...
    if (value === 'new') {
      ...
      // Keep current config as template
    } else {
      ...
      if (cred) {
        ...
        // Only load if provider matches or if it's generic?
        // Actually, we should check if the credential provider matches the selected provider
        // If not, maybe warn? Or switch provider?
        // Switch provider if possible
        if (safeData.provider && safeData.provider !== provider) {
          setProvider(safeData.provider);
        }
        ...
        updateBlock(block.id, {
          properties: {
            ...properties,
            ...data,
            host: safeData.sshHost || safeData.host,
            port: safeData.sshPort || safeData.port,
            isConnected: false, // Reset connection status
          }
        });

        // Update local state
        if (safeData.baseUrl) setBaseUrl(safeData.baseUrl);
        if (safeData.clientId) setClientId(safeData.clientId);
        if (safeData.clientSecret) setClientSecret(safeData.clientSecret);
        if (safeData.username) setUsername(safeData.username);
        if (safeData.sshHost) setSSHHost(safeData.sshHost);
        if (safeData.sshPort) setSSHPort(safeData.sshPort);
        if (safeData.authMethod) setAuthMethod(safeData.authMethod);
        
        // Load secrets if available
        if (safeData.accessToken) setAppPassword(safeData.accessToken);
        if (safeData.privateKey) setPrivateKey(safeData.privateKey);
        if (safeData.passphrase) setSSHPassphrase(safeData.passphrase);
        if (safeData.password) setSSHPassword(safeData.password);

        toast({
          title: 'Loaded',
          description: `Loaded credential: ${cred.name}`,
        });
      }
    }
  };
  ...
  const updateProperties = useCallback((updates: Partial<CloudBlockProperties>) => {
    updateBlock(block.id, {
      properties: {
        ...properties,
        ...updates,
  ...
  const handleProviderChange = (newProvider: CloudBlockProperties['provider']) => {
    ...
    // Save credential
    saveCredential({ provider: newProvider });
  };
  ...
  const loadIntegration = useCallback(async () => {
    ...
    try {
      ...
      // Update local state from integration data on load
      if (integration.autoRefresh !== undefined) {
        setAutoRefresh(integration.autoRefresh);
      }
      if (integration.cronSchedule !== undefined) {
        setCronSchedule(integration.cronSchedule);
      }
      if (integration.cronTimezone !== undefined) {
        setCronTimezone(integration.cronTimezone);
      }

      // Update block properties with data from backend to ensure sync
      // Use JSON.stringify for deep comparison to avoid infinite loop
      if (JSON.stringify(block.properties) !== JSON.stringify({ ...block.properties, ...integration })) {
        updateBlock(block.id, {
          properties: {
            ...block.properties,
            ...integration,
            // Don't overwrite properties that weren't returned or are managed locally first
      ...
      console.error('Failed to load cloud integration', error);
      // Ignore 404s (no integration yet)
    } finally {
      setLoadingIntegration(false);
    }
  }, [chatbotId, token, block.id, updateBlock]);

  // Load integration on mount
  useEffect(() => {
    if (chatbotId && token && block.id && loadedBlockIdRef.current !== block.id) {
      loadedBlockIdRef.current = block.id;
      loadIntegration().then(() => {
        // Initial sync of cron settings after load
        // We removed the redundant sync here because loadIntegration now handles it
        // and syncing from block.properties again could cause race conditions
      });
    }
  }, [chatbotId, token, block.id, loadIntegration]);
      ...
      if (authMethod === 'app_password') {
        ...
        try {
          ...
          if (testResult.connected) {
            ...
            // Explicitly update backend status to ensure persistence
            // This prevents loadIntegration from reverting the state later
            await updateCloudIntegration(
              block.id,
              { isConnected: true },
              token || ''
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Credential Selection - Show only if provider is selected and not OAuth-only providers */}
          {provider && provider !== 'googledrive' && provider !== 'onedrive' && (
            <div className="space-y-2 pt-2 border-t">
              {isConnected ? (
                <div className="flex flex-col gap-2 p-3 bg-muted rounded-md border">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="h-4 w-4 text-green-600" />
                    <span className="text-sm font-medium">Using saved credentials</span>
                  </div>
                  <div className="text-sm pl-6 font-semibold">
                    {selectedCredentialId === 'new' ? 'Unsaved Credential' : credentials.find(c => c.id === selectedCredentialId)?.name || 'Custom Configuration'}
                  </div>
                  <Button 
                    variant="outline" 
                    size="sm"
                    ...
                    onClick={() => {
                      handleDisconnect();
                      // Also reset selection to allow choosing new creds
                      // setSelectedCredentialId(''); 
                      // Don't reset immediately, user might just want to edit?
                      // User said "Select different credentials", so probably reset is good
                    }}
                  >
                ...
                <>
                  ...
                  <Select value={selectedCredentialId} onValueChange={handleCredentialSelect}>
                    ...
                    <SelectContent>
                      ...
                      {credentials
                        // Strict filter by provider
                        .filter(c => c.data.provider === provider)
                        .map(c => (
                    ...
                    <div className="space-y-2 mt-2">
                      ...
                      <div className="flex gap-2">
                        <Input 
                          ...
                          placeholder="e.g. Corporate Nextcloud"
                          className="flex-1"
                        />
          ...
          {/* Google Drive/OneDrive - always show connection info (no credential object needed) */}
          {(provider === 'googledrive' || provider === 'onedrive') && (
            <div className="pt-2 border-t">
               {/* Content logic handled below */}
            </div>
          )}

          {/* Show form only if NOT connected (to edit details) */}
          {/* OR if it's an OAuth provider which has its own connected UI flow */}
          {(!isConnected && (selectedCredentialId || provider === 'googledrive' || provider === 'onedrive')) && (
            <>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        saveCredential({ authMethod: value });
                      }}
                      disabled={isConnected}
                    >
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            // updateProperties({ username: e.target.value }); // Don't update parent on change
                          }}
                          onBlur={() => {
                            updateProperties({ username });
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onBlur={() => saveCredential({ accessToken: appPassword })}
                          disabled={isConnected}
                        />
                        <p className="text-xs text-muted-foreground">
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onBlur={() => {
                        updateProperties({ username: sshUsername });
                        saveCredential();
                      }}
                      disabled={isConnected}
                    />
      ...
      {isConnected && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button
              onClick={() => setIsIndexedFilesModalOpen(true)}
              variant="outline"
              className="w-full"
            >
              <ExternalLink className="w-4 h-4 mr-2" />
              View Indexed Files
              {indexedFileCount > 0 && ` (${indexedFileCount})`}
            </Button>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) => {
                  ...
                  // Also save credential if one is selected
                  if (selectedCredentialId) {
                     saveCredential({ autoRefresh: checked });
                  }
                }}
              />
            </div>
      ...
      {/* Indexed Files Modal */}
      {isConnected && (
        <CloudIndexedFilesModal
          open={isIndexedFilesModalOpen}
          onOpenChange={setIsIndexedFilesModalOpen}
          chatbotId={chatbotId || ''}
          blockId={block.id}
          token={token || ''}
        />
