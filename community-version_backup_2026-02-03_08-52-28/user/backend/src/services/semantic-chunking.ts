...
export class SemanticChunkingService {
  ...
  private getEmbeddings(): OpenAIEmbeddings | null {
    if (!this.embeddings) {
      ...
      if (!apiKey) {
        logger.warn('OpenAI API key not found, semantic similarity will be disabled', {
  ...
  private async calculateSemanticSimilarity(text1: string, text2: string): Promise<number> {
    try {
      ...
      if (!embeddings) {
        logger.warn('OpenAI embeddings not available, skipping semantic similarity calculation', {
          service: 'semantic-chunking',
        });
        return 0;
      }
      ...
      logger.warn('Failed to calculate semantic similarity', {
        error: error instanceof Error ? error.message : String(error),
        service: 'semantic-chunking',
      });
      return 0;
