...
export class SemanticChunkingService {
  ...
  private getEmbeddings(): OpenAIEmbeddings | null {
    if (!this.embeddings) {
      ...
      if (!apiKey) {
        console.warn('OpenAI API key not found, semantic similarity will be disabled');
        return null;
      }
  ...
  private async calculateSemanticSimilarity(text1: string, text2: string): Promise<number> {
    try {
      ...
      if (!embeddings) {
        console.warn('OpenAI embeddings not available, skipping semantic similarity calculation');
        return 0;
      }
