...
const corsApiLogger = logger.child({ service: 'admin-backend', component: 'corsApi' });
...
async function getAllowedOrigins(chatbotId: string): Promise<string[]> {
  try {
    ...
    corsApiLogger.error('Error fetching allowed origins', { error: error instanceof Error ? error : new Error(String(error)) });
