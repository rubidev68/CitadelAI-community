...
const semanticChunkingLogger = logger.child({ service: 'admin-backend', component: 'semanticChunking' });
...
export class SemanticChunkingService {
   ...
   */
  private getEmbeddings(): OpenAIEmbeddings | null {
    if (!this.embeddings) {
      const apiKey = config.OPENAI_API_KEY;
      if (!apiKey) {
        semanticChunkingLogger.warn('OpenAI API key not found, semantic similarity will be disabled');
        return null;
      }
      ...
      try {
        ...
        semanticChunkingLogger.warn('Failed to initialize OpenAI embeddings', { error: error instanceof Error ? error : new Error(String(error)) });
        return null;
      }
    }
  ...
  private async calculateSemanticSimilarity(text1: string, text2: string): Promise<number> {
    try {
      ...
      if (!embeddings) {
        semanticChunkingLogger.warn('OpenAI embeddings not available, skipping semantic similarity calculation');
        return 0;
      }
